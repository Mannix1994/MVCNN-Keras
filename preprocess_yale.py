...
DATA_DIR = os.path.join('data', 'CroppedYale')
...
def main(argv):
    ...
    test_list = []
    face_dataset = []
    objects_dir = os.path.join(tmp, 'CroppedYale')
    ...
    for o_id, d in enumerate(object_dirs):
        ...
        if os.path.isdir(object_dir):
            image_files = sorted(os.listdir(object_dir))
            image_files = [i_f for i_f in image_files if i_f.endswith('.jpg')]
            resize_and_save(DATA_DIR, o_id, object_dir, 'train', image_files, train_list, face_dataset)
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        print("python preprocess_modelnet40v1.py data/CroppedYale.zip")
        exit(-1)
    else:
        main(sys.argv)
